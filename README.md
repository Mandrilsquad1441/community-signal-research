...
The first targeted [post-v3 development stress](evals/evidence/FAILED-DEVELOPMENT-STRESS-V4-1.md) also failed and is preserved rather than cherry-picked away: one quote replicate changed capitalization, and both privacy reviewers agreed that three private-source replicates disclosed record-specific facts in narrative fields. A second [development stress](evals/evidence/FAILED-DEVELOPMENT-STRESS-V4-2.md) eliminated fabricated substrings but still found three clause-fragment citations and two unanimous privacy failures. A third [development stress](evals/evidence/FAILED-DEVELOPMENT-STRESS-V4-3.md) fixed the complete-sentence failure, but two responses had deterministic private-citation defects and both reviewers rejected every private-source replicate. The fourth [development stress](evals/evidence/FAILED-DEVELOPMENT-STRESS-V4-4.md) made every private citation exact, but one quote response regressed to clause fragments, only three of five private responses used the required WTP template, and privacy reviewers rejected four and five of five private responses. A new fixed-string private-response successor is required before confirmatory v4.
