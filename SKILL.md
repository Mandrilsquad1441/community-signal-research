...
3. **Public excerpts are copy-only values.** Copy each one directly from `captured_text`, then require the exact case- and punctuation-sensitive condition `excerpt in captured_text` and at most 25 words before emitting it. Never retype, normalize, repair, add, drop, or change a word.
4. **Supplied-private text is analysis-only.** Across the entire public response—not only citations—expose only source IDs, opaque record references, caller-declared file hashes, null excerpts, controlled category membership, and aggregate labels/counts. Never reveal a record-specific fact through quotation, summary, close paraphrase, a limitation, or next-test wording.
...
- **Literal-citation pass:** Construct every direct public excerpt by copying a contiguous span from that source's `captured_text`, not by retyping a remembered phrase. Prefer the complete shortest sentence when it fits the 25-word ceiling. Immediately before returning, compare character for character, including capitalization, apostrophes, and punctuation: the exact source text must contain the exact excerpt. If it does not, replace it with a copied exact span or fail closed; never add an article, repair grammar, change capitalization, or put an ellipsis inside the quote. Generated findings may apply only the documented deterministic Markdown escaping and NFKC/whitespace display normalization after the underlying ledger excerpt passes this check.
- **Supplied-private output pass:** Treat `captured_text`, `source_context`, publication time, per-record flags, and evidence-type detail for every `supplied_private` row as non-exportable. Draft descriptive prose from public sources and the predeclared decision/hypothesis first. Then scan every output string—including the memo, WTP and counterevidence summaries, limitations, next tests, and explanations—and delete any wording or fact learned from a private record. Do not attribute a fact to private evidence even when a public source states a similar fact; cite the public source for the fact and mention the private aggregate separately. Private evidence may affect only source-ID membership, aggregate author/thread/support/WTP category or count, and an opaque citation with record reference, caller-declared file hash, and null excerpt. Use an accurate fact-free form such as "Two supplied-private sources count toward eligible support; record-specific details withheld," "One supplied-private source meets the explicit purchase-intent criterion; record-specific details withheld," or "Supplied-private counterevidence is present; record-specific details withheld." If no generic form is accurate, state only that record-specific details are withheld.
