...
The canonical Codex operator additionally records its script hash, resolved executable and binary hash, CLI version, the canonical selected-model entry and its binding hash, an informational full-catalog hash, a model-visible prompt-isolation probe hash, repository state, structured host OS name and platform string, disabled features, timeout, bounded parallelism, and child-process containment mode. Verification requires the structured OS name and containment mode to match the host performing the bound run verification.
...
Run the production batch directly in one attended foreground terminal. Detached launchers, `Start-Process`, `Start-Job`, scheduler wrappers, and closing the supervising terminal are outside this protocol. Preserve the operator's ten-second heartbeats with the command output. Use conservative bounded parallelism for the host; two workers is the default.
...
- an existing `operator-config.json`, `operator-summary.json`, `operator-abort.json`, `operator-abort-cleanup.json`, or prior per-trial prompt/execution/log/output file;
...
The Codex invocation ignores user configuration and repository rules, skips host-skill discovery, disables host and bundled skill-instruction injection, inherits no shell environment through the model configuration, and disables model-callable shell, browser/search, connectors/apps/plugins, memories, computer use, multi-agent, and the other features enumerated in `run_trials.py`. Before accepting the executable identity, the operator runs `codex debug prompt-input` under the same skill/feature controls, hashes its canonical model-visible messages, requires its marker, and fails closed on a host-skill catalog or skill path. This probe binds observable prompt composition for the captured CLI/configuration; it does not prove that no local metadata scanner touched an ignored file. The Codex service still requires its normal host connectivity; the claim is that the evaluated model receives no enabled retrieval or execution tool.

Both conditions use the same executable, model, reasoning effort, low verbosity, sandbox, timeout, and disabled-feature set. The operator hashes the executable before its capability probes and requires that hash to remain stable after version/help/prompt/model-catalog probes, immediately before and after each process launch, after each process completes, and after the batch. A mismatch invalidates the first attempt rather than silently rebinding configuration. The canonical hash of the selected model entry is the model-catalog binding; the raw full-catalog hash is retained only as environment telemetry because unrelated catalog entries may change. The operator hashes captured `codex exec --help`, records any detected `--seed` or `--request-seed` flags, and refuses preflight/batch execution if either appears until the operator can apply allocated pair seeds. For a permitted run, config and every execution record use one canonical declaration that pair seeds were recorded but not applied, and seed flags are forbidden in recorded argv. Independent replicates therefore remain necessary.
...
The operator submits at most `--jobs` trials at a time and emits ten-second liveness heartbeats. Before `Popen` can create a child, a pending launch slot is inserted into the shared owner registry; process creation and publication run in a short interrupt-deferred critical section, so a catchable interruption cannot create an unrecorded child between a call return and local assignment. On Windows, every Codex CLI invocation, including version/help/prompt/model probes and smoke, is created with `CREATE_SUSPENDED`, assigned to a dedicated nested kill-on-close Job Object before target code can execute, and resumed only after the operator finds exactly one primary thread, verifies that thread's process ownership, and requires `ResumeThread` to return the expected prior suspend count of one. Normal completion, timeout, cancellation, and operator failure use Job termination, root reaping, Job Basic Accounting drain verification through `ActiveProcesses == 0`, and Job-handle close. No detached or breakaway fallback is allowed. This is ordinary local process-tree cleanup for direct `CreateProcess` descendants while breakaway remains disabled; it is not a hostile same-user security sandbox and cannot confine work delegated to an external process or service.

On POSIX, `start_new_session=True` creates a new session and original process group for the CLI. Cleanup sends `SIGKILL` to that original group and polls `killpg(..., 0)` until `ESRCH`, with other errors fatal. This is a cooperative boundary whose recorded policy sets `escape_resistant=false`: the trusted CLI and its descendants must not call `setsid`, call `setpgid` to leave the original group, or delegate process creation to an external service. Such re-sessioning, re-grouping, or delegation can escape the cleanup proof. The supported Python standard library has no portable hard process-tree cage across POSIX hosts, and this protocol does not claim one. On either platform, inability to establish the recorded boundary, terminate it, reap the root, prove that boundary drained within the recorded five-second cleanup bound, or complete required handle cleanup is fatal. Workers attempt the full drain and close, but the shared registry retains any unproven boundary; after worker shutdown, the supervisor retries and requires the registry to be empty on both normal and abort paths.

The persistent trial directory records the sent prompt, raw response when present, Codex JSONL event stream, stderr, and start/final execution records. Those records bind the allowed-input and prompt hashes plus the response, stdout, and stderr hashes. `operator-config.json` binds the run configuration before dispatch; `operator-summary.json` records every trial outcome. Config, sent prompt, start/final records, summary, and abort records are flushed and file-synced in causal order. On POSIX, every durable exclusive evidence-file creation is followed by an `fsync` of its parent directory. The log handles and a present child-created response are file-synced before the final record hashes them; the final record's parent-directory `fsync` then seals that trial-directory state. On Windows, Python's `os.fsync` uses the CRT `_commit` operation for the file descriptor, but the operator performs no separate parent-directory flush and makes no stronger Windows directory-entry durability claim. Every evidence file is opened in exclusive-create mode; a pre-existing path aborts and is preserved. A timeout, nonzero exit, or cleanly launched process with no answer remains an observed model outcome. A pre-launch operator error, launch error, missing execution record, or incomplete hash chain makes the run ineligible for blinding; never repair or replace such a trial post hoc.

On a catchable interruption after config sealing, the operator best-effort writes an exclusive, flushed and file-synced `operator-abort.json`, cancels queued work, requests termination of every active recorded boundary, and waits for worker cleanup. If that cleanup is not provable, an exclusive durable `operator-abort-cleanup.json` records the failures known at creation without overwriting the primary marker. These records do not expand the platform containment scope described above. An abrupt host shutdown can prevent either write. Either abort marker or an `operator-config.json` without one complete valid `operator-summary.json` permanently disqualifies the attempt; never resume it or synthesize missing records. Disclose infrastructure exclusions separately from behavioral outcomes. The treatment-blind record for one such event is [`evidence/ABORTED-CONFIRMATORY-V2.md`](evidence/ABORTED-CONFIRMATORY-V2.md).
