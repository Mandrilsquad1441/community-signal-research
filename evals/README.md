...
A complete run can also be protocol-valid yet fail an outcome gate. [Confirmatory attempt v3](evidence/FAILED-CONFIRMATORY-V3.md) is preserved in that category: all protocol gates passed, but two of 60 skill trials violated the preregistered zero-critical-failure requirement. Its [exact report](evidence/confirmatory-v3-report.json) remains negative evidence and cannot be repaired or rescored into a release pass.

The first post-v3 [development stress](evidence/FAILED-DEVELOPMENT-STRESS-V4-1.md) also failed its predeclared targeted checks: one of five quote trials changed the capitalization of a public excerpt, and two independent reviewers agreed that three of five private-source trials leaked record-specific facts through narrative fields. It is non-confirmatory development evidence and forces a new candidate, plan, paths, and seed.

The second [development stress](evidence/FAILED-DEVELOPMENT-STRESS-V4-2.md) eliminated fabricated excerpts but still failed: three responses copied exact clause fragments instead of complete fitting sentences, and both reviewers found private disclosure in two replicates (reviewer A found one additional conservative failure). It likewise cannot become release evidence.

The third [development stress](evidence/FAILED-DEVELOPMENT-STRESS-V4-3.md) made all ten targeted public citations exact complete fitting sentences, but still failed: two private citations violated the deterministic provenance contract, and both independent reviewers returned `PRIVATE_DISCLOSURE` for all five private-source replicates. It forces another candidate and cannot become release evidence.

The fourth [development stress](evidence/FAILED-DEVELOPMENT-STRESS-V4-4.md) made every targeted private citation exact and retained the fixed safe next test, but still failed: one quote response copied three clause fragments despite complete fitting sentences, only three of five private responses used the required WTP template, and the two independent reviewers found disclosure in four and five of five private-source replicates. It forces a fixed-string private-response successor and cannot become release evidence.
