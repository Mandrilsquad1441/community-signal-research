...
Allowed decisions are `same_source` and `independent`. `same_source` collapses the pair. For short-exact review, `independent` clears only the pair after the reviewed IDs are mapped to their final groups; independence is not transitive. A fuzzy warning is cleared only by a review naming that exact warned source-ID pair. Exact normalized text is collapsed automatically only at **80 or more characters and 12 or more words**. Exact normalized text at **20 or more characters and 4 or more words** that fails either hard condition is not auto-collapsed without a reviewed dependency or another hard identity. An all-public unresolved pair produces `POSSIBLE_SHORT_EXACT_DUPLICATE`; a pair with any non-public member produces the generic `NON_PUBLIC_DUPLICATE_REVIEW_REQUIRED`, which names only the source-ID pair and withholds the comparison mechanism. Canonical unit URLs, URL-derived native identities, repeated private provenance, and explicit reposts also collapse. The auditor completes all hard and transitive unions before selecting short-exact work. A match already in one final group does not warn. Each short-text class with any unresolved final-group pair emits exactly one deterministic pair warning per run; add that pair's decision and rerun until all pairs are reviewed. Warning paths and generated artifacts remain deterministic under ledger-row permutations. Generated audit and findings artifacts disclose each all-public group's origin, members, and reasons. A group containing a non-public member instead sets `origin_source_id` to null, exposes `representative_source_id` as the lexical source-ID representative, retains source-ID membership and aggregate counts, and uses `non_public_details_withheld`; chronology and record-specific collapse details remain private. Findings label that column **Representative**, not **Origin**, whenever such a group is present.

The optional fuzzy scan considers records with at least 30 tokens using five-token shingles and a 90% length/similarity threshold. It has deterministic ceilings of 100,000 candidate pairs, 5,000,000 shingle lookups, and 200,000 stored shingle digests. Exceeding any ceiling produces `FUZZY_SCAN_SKIPPED`, a strict warning requiring external blocking/review instead of unbounded work. All-public candidate warnings may disclose the similarity percentage. If either member is non-public, the public warning is `NON_PUBLIC_DUPLICATE_REVIEW_REQUIRED` and withholds the mechanism and percentage; scan-budget warnings likewise omit private-derived work counts whenever a non-public row is present.
...
- `audit.json`: stable issue codes, coverage, signal metrics, `source_groups_after_collapse`, `collapsed_duplicate_groups`, privacy-filtered duplicate-group details, tool version, semantic input fingerprint, and output hashes. It is written last as the success commit marker.
- `signals.csv`: ranked metrics with spreadsheet-formula protection.
- `findings.md`: deterministic report with short public excerpts, excluded promotional context, counterevidence, full scope/query disclosure, limitations, and fingerprint. For a supplied-private source, the renderer withholds its excerpt, publication date, per-record flags, evidence-type detail, and every record-specific private-text fact; it may show only the source ID, opaque record locator, caller-declared source-file SHA-256, controlled source-ID category membership, aggregate labels/counts, and the unauthenticated-digest caveat. Researcher-authored hypotheses, inferences, recommendations, limitations, and next tests still require the same response-boundary information-flow review before publication because the offline overlap scan cannot prove semantic non-disclosure.
...
The public semantic fingerprint binds every ledger row whose `visibility` is not exactly `public` to only four source entries: a syntactically valid source ID, a syntactically valid opaque non-personal `record_ref`, a syntactically valid caller-declared `source_file_sha256`, and `excerpt: null`. Malformed allowlist values become null. It excludes every other per-record field, including schema version, visibility spelling, platform/community, URLs and IDs, dates, author/language/status, flags, stance, evidence types, engagement, repost/review linkage, signal/query back-links, notes, and private text. Controlled source-ID category membership remains bound through signal citations and query inclusions. Aggregate labels/counts are deterministic derived outputs rather than additional fingerprint inputs; `artifact_hashes` binds their rendered `signals.csv` and `findings.md` values. Rows with malformed or missing intended-private visibility fail closed to this branch. Public rows remain fully bound byte-for-byte under the existing canonical ordering.

Separately, every free-text field that can flow into public artifacts is checked for ASCII-shaped email patterns, and non-URL fields are checked for ASCII-shaped phone patterns. The regex character/digit/boundary classes are explicit rather than runtime Unicode `\w`, `\d`, `\b`, or case folding. For private title/text/excerpt/notes, the scan indexes every four-token window; a one-to-three-token field of at least eight normalized characters is indexed as a whole. Alphanumeric private identifier components of at least 12 characters are additionally compared with output IDs, opaque record references, and public URLs. The scan refuses more than 200,000 distinct digests. It is intentionally conservative and bounded: generic phrases can produce false positives, while Unicode-formatted contact data, paraphrases, alternate encodings, short secrets, addresses, and unrecognized identifier forms can evade it. It does not replace a response-boundary information-flow review before publication; absence of a scan hit is not permission to paraphrase a private record.
