...
The excerpt must be a literal, short substring of `captured_text` within both the 25-word and 500-character ceilings. Build and lock citations before drafting narrative text. Mark sentence boundaries first. When a relevant source sentence is 25 words or fewer, copy that complete sentence from its first character through its sentence-final punctuation; a comma, semicolon, colon, or dash is not a sentence boundary, so extend through the source's `.`, `?`, or `!`. Use a shorter contiguous span only when no complete relevant sentence fits. Paraphrases belong in the hypothesis, not the excerpt. Before publication, require the exact case- and punctuation-sensitive condition `excerpt in captured_text`; do not grammar-correct, add an article, lowercase the first word, change capitalization, drop a word, or append an ellipsis inside the quoted value. Generated Markdown may apply only the documented deterministic escaping and NFKC/whitespace display normalization, so compare the underlying ledger value rather than the escaped Markdown bytes and confirm that the visible wording is not semantically altered. Store engagement only as timestamped context; the scoring algorithm ignores it.
...
- Supplied-private signal evidence and provenance: retain the source in the ledger and signal citations, but do not put it in research-note observations. Freeze every prose field in a public-only draft while private text and record metadata are `[SEALED]`; this includes limitations and next tests. Then reduce each private row to a structured patch containing only source ID, opaque record reference, caller-declared file hash, and controlled support/counter/WTP booleans. Apply that patch only to structured memberships, aggregate counts/labels, WTP basis, opaque citations, and the applicable exact fact-free templates; never edit prose afterward. Outside an exact template, never make a private source the subject of a descriptive clause or say what private evidence indicates, contains, lacks, does, or does not supply. Do not combine public and private attribution for a descriptive fact. Before release, delete all private content counterfactually; any changed non-template wording is a privacy failure.
