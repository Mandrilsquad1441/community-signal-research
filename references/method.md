...
- Supplied-private signal evidence and provenance: retain the source in the ledger and signal citations, but do not put it in research-note observations. Enter `PRIVATE_SAFE_MODE` before reading private free text. Copy each permitted citation object into `PRIVATE_CITATION_MAP` exactly—including all 71 characters of `sha256:` plus 64 lowercase hex digits—then freeze all response strings in a public-only `PUBLIC_DRAFT`. Reduce private content to `PRIVATE_PATCH` and apply only the structured-path allowlist and exact required-field templates in `SKILL.md`; `public_memo`, limitations, and the fixed safe next test never receive private wording. Before release, counterfactually seal all private content and recompare every citation with the source row character for character. Any changed non-template string or citation mismatch is a privacy failure.
