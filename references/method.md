...
The excerpt must be a literal, short substring of `captured_text` within both the 25-word and 500-character ceilings. Build and lock citations before drafting narrative text. When a relevant source sentence is 25 words or fewer, copy that complete sentence from its first character through its punctuation; use a shorter contiguous span only when no complete relevant sentence fits. Paraphrases belong in the hypothesis, not the excerpt. Before publication, require the exact case- and punctuation-sensitive condition `excerpt in captured_text`; do not grammar-correct, add an article, lowercase the first word, change capitalization, drop a word, or append an ellipsis inside the quoted value. Generated Markdown may apply only the documented deterministic escaping and NFKC/whitespace display normalization, so compare the underlying ledger value rather than the escaped Markdown bytes and confirm that the visible wording is not semantically altered. Store engagement only as timestamped context; the scoring algorithm ignores it.
...
- Supplied-private signal evidence and provenance: retain the source in the ledger and signal citations, but do not put it in research-note observations. Partition before drafting: build a public fact bank from caller framing and public sources, and reduce each private row to a sealed tuple of source ID, opaque record reference, caller-declared file hash, and controlled support/counter/WTP booleans. At drafting time act as though all other private text and metadata are `[SEALED]`. Private evidence may affect only structured source membership, aggregate classifications/counts, and opaque citations with null excerpts. Every descriptive clause in a memo, summary, exclusion, limitation, recommendation, explanation, or next test must instead trace to public evidence or caller-declared framing. Use only a fixed fact-free aggregate form such as "One supplied-private source meets the explicit purchase-intent criterion; record-specific details withheld." Before release, counterfactually delete all private content; if any natural-language value would change, delete or rewrite it from the public fact bank.
