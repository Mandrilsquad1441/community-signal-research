...
Document provenance and the transformation into ledger records. Use `platform: "export"`, `source_type: "export_record"`, `capture_method: "export"`, `visibility: "supplied_private"`, null URLs, an opaque non-personal `record_ref`, and the authorized source file's caller-declared SHA-256. The helper cannot authenticate that digest against a file it never receives. Redact personal information and state that remote-source verification was unavailable. Enter `PRIVATE_SAFE_MODE` before reading private free text. Copy the source ID, opaque locator, and all 71 hash characters directly into a locked `PRIVATE_CITATION_MAP`; never retype or abbreviate them. Freeze every response string from caller framing and public evidence, then apply only the structured `PRIVATE_PATCH` paths and exact required-field templates in `SKILL.md`. The public memo, limitations, and fixed safe next test cannot receive private-derived wording. Run both the deletion gate and exact citation comparison before release. Generated findings also use a bounded hashed overlap scan, but neither that scan nor a caller-declared digest proves semantic non-disclosure or file authenticity, so review generated findings at the response boundary before publication.
