...
Document provenance and the transformation into ledger records. Use `platform: "export"`, `source_type: "export_record"`, `capture_method: "export"`, `visibility: "supplied_private"`, null URLs, an opaque non-personal `record_ref`, and the authorized source file's caller-declared SHA-256. The helper cannot authenticate that digest against a file it never receives. Redact personal information and state that remote-source verification was unavailable. Treat private text and record-specific metadata as analysis-only. Draft descriptions from public evidence first; add private evidence afterward only as a separate, fact-free aggregate statement. Public-facing output may expose only the source ID, opaque record reference, caller-declared hash, null excerpt, controlled source-ID category membership, and aggregate labels/counts. Withhold private publication dates, per-record flags, evidence-type detail, quotations, summaries, close paraphrases, and all other record-specific private-text facts from a memo, WTP or counterevidence summary, limitation, next test, or explanation. A safe form is "One supplied-private source meets the explicit purchase-intent criterion; record-specific details withheld." Generated findings withhold private excerpts and the listed private metadata, while a bounded hashed overlap scan checks public-output text, URLs, IDs, and locators for exact or near-exact leakage; that scan does not prove semantic non-disclosure, so review generated findings at the response boundary before publication.
