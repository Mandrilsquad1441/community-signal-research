...
Document provenance and the transformation into ledger records. Use `platform: "export"`, `source_type: "export_record"`, `capture_method: "export"`, `visibility: "supplied_private"`, null URLs, an opaque non-personal `record_ref`, and the authorized source file's caller-declared SHA-256. The helper cannot authenticate that digest against a file it never receives. Redact personal information and state that remote-source verification was unavailable. Before drafting, build a public fact bank and reduce private rows to sealed provenance plus controlled support/counter/WTP membership. Treat every other private value as `[SEALED]`. Public-facing output may expose only the source ID, opaque record reference, caller-declared hash, null excerpt, controlled source-ID category membership, aggregate labels/counts, and an unextended fixed fact-free aggregate template. Every descriptive clause—including a memo, WTP or counterevidence summary, exclusion, limitation, recommendation, next test, or explanation—must trace to public evidence or caller-declared framing. Run a counterfactual deletion gate: if removing private text changes any natural-language value, delete or rewrite it from public evidence. A safe complete form is "One supplied-private source meets the explicit purchase-intent criterion; record-specific details withheld." Generated findings withhold private excerpts and the listed private metadata, while a bounded hashed overlap scan checks public-output text, URLs, IDs, and locators for exact or near-exact leakage; that scan does not prove semantic non-disclosure, so review generated findings at the response boundary before publication.
